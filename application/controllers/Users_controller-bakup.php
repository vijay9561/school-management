...
class Users_controller  extends MY_Controller{
  ...
  public function registration_process(){
	  ...
	  if($result==1){
		    $name=$_POST['Principle_name'];
			$password=$_POST['Principle_name'];
			$email=$_POST['email'];	  
	        $mail_subject="VMBSS-Your School Registration Successfully";
...
padding-bottom: 3px;">
             ...
             <div style="color:#555555;font-family:Droid SerifGeorgia, Times,Times New Roman,serif; padding-right: 10px; padding-left: 10px; padding-top: 10px; margin-top:-50px;">
                        <div style="font-size:12px;line-height:24px;color:#555555;font-family:Droid SerifGeorgia, Times,Times New Roman,serif;text-align:left;">
                          <span style="font-size:16px;line-height:19px; margin-left:20px;">
						    <p style="font-size: 14px;text-align:left;"><strong><span style="font-size:18px;">Respected '.$name.',</strong></p>
                          <p style="color: #000000; text-align:justify; font-size:16px;">
	You have been Registered Successfully. Now you need to login to the system with your login credential details
	                        <p style="line-height:11px;color:#000;"><strong>Email ID :</strong> '.$email.'</p>
	                        <p style="line-height:11px;color:#000;"><strong>Password :</strong> '.$password.'</p>
							<p style="color:#000;"><strong>Note:</strong> if your account activate please contact to this number: <strong>7448286205</strong></p>	
						  </p>
                        </div>
                      </div>
                    </div>
  ...
  $from_email = "info@vmbss.org";
        ...
        $this->email->from($from_email, 'VIMLAI SCHOOL MANAGEMENT');
        $this->email->to($email);
...
$mail_to1='vijay171819@gmail.com';
...
public function submit_bonafied_student_data(){
 ...
 public function submit_nirgam_uttra(){
   ...
   public function submit_applications_leaving(){
	...
	$this->db->query("delete from notification where parent_id='$get_adhar->adhar_card'");
...
public function all_student_attendance_submit(){
  ...
  public function add_student_fees(){
    ...
    $insert_array=array('total_fees'=>$total_fees,'total_discount_fees'=>$total_discount_fees,'dicount_percentage'=>$dicount_percentage,'pid'=>$pid,'division'=>$division,'parent_id'=>$parent_id,'class'=>$class,'create_date'=>date('Y-m-d'));
	 ...
	 $total_fees_array=$_POST['total_fees'];
	 $paid_fees_array=$_POST['paid_fees'];
	 $remaining_fees_array=$_POST['remaining_fees'];
...
$insertdata=array('fees_type'=>$fees_type,'total_fees'=>$total_fees,'paid_fees'=>$paid_fees,'remaining_fees'=>$remaining_fees,'fees_id'=>$id,'created_date'=>date('Y-m-d'));
	...
	// $id=$this->db->insert_id();
	 ...
	 //exit;
       ...
       $insertdata=array('fees_type'=>$fees_type,'total_fees'=>$total_fees,'paid_fees'=>$paid_fees,'remaining_fees'=>$remaining_fees);
