...
$mysqluery=$this->db->query("select *from tbl_main_slider  where (slider_Heading LIKE '%".$searchid."%' OR slider_small_heading LIKE '%".$searchid."%') and slider_type='supper'")->result();
//$mysqluery=mysql_query($query);
}else{
$mysqluery=$this->db->query("select *from tbl_main_slider where slider_type='supper'")->result();
//$mysqluery=mysql_query($query);
}
		 ?>
		 <script>
...
<input type="submit" class="btn btn-primary" value="Search" onclick="return search_result()" />
		...
		<div class="table-responsive">
			...
			<a href="#" onclick="return deletepincodes(<?php echo $row->mid; ?>)" class="btn btn-danger" title="Delete"><span class="glyphicon glyphicon-trash"></span></a></td>
                        </tr>
						<?php $i++; } ?>
                      </tbody>
                    </table>  
