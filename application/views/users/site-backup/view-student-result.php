...
if(isset($_SESSION['PRINCIPLE_ID'])) { 
 ...
 $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where (r.class='$search' || r.division='$search' || r.exam_name like '%".$search."%' || p.Student_name like '%".$search."%' ||  p.adhar_card like '%".$search."%') and r.pid='".$pid."' order by r.rmid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
 }else{
 $pid=$_SESSION['PRINCIPLE_ID'];
 $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where (r.class='$search' || r.division='$search' || r.exam_name like '%".$search."%' || p.Student_name like '%".$search."%' ||  p.adhar_card like '%".$search."%') and  r.pid='".$pid."' order by r.rmid LIMIT ".$pageLimit." , ".$setLimit)->result();
 } 
 ...
 $pid=$get_pid->Pid;
  $class=$get_pid->schools_name;
   ...
   $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where (r.class='$search' || r.division='$search' || r.exam_name like '%".$search."%' || p.Student_name like '%".$search."%' ||  p.adhar_card like '%".$search."%') and  r.pid='".$pid."' and r.class='$class' and r.division='$division' order by r.rmid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
 ...
 $pid=$get_pid->pid;
  $class=$get_pid->Student_class_division;
   $division=$get_pid->divsion;
    $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where  r.pid='".$pid."' and r.class='$class' and r.division='$division' order by r.rmid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
   }  
  }else{
if(isset($_SESSION['PRINCIPLE_ID'])) { 
 if($_SESSION['USERTYPE']=='clerk'){ 
 $get_pid=$this->db->query("select login_id from tbl_principle where Pid='".$_SESSION['PRINCIPLE_ID']."'")->row();
 $pid=$get_pid->login_id;
 $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where  r.pid='".$pid."' order by r.rmid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
 }else{
 $pid=$_SESSION['PRINCIPLE_ID'];
 $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where  r.pid='".$pid."' order by r.rmid LIMIT ".$pageLimit." , ".$setLimit)->result();
 } 
 }elseif(isset($_SESSION['TEACHER_ID'])){
 $get_pid=$this->db->query("select Pid,schools_name,divsion from tbl_teacher where Tid='".$_SESSION['TEACHER_ID']."'")->row();
 $pid=$get_pid->Pid;
 ...
 $pid=$get_pid->pid;
  $class=$get_pid->Student_class_division;
   $division=$get_pid->divsion;
    $mysl_query=$this->db->query("select *from result_master r inner join tbl_parent p on r.parent_id=p.Ptid where r.parent_id='".$_SESSION['PARENT_ID']."' and  r.pid='".$pid."' and r.class='$class' and r.division='$division' order by r.rmid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
 }  
 }
?>
<?php
