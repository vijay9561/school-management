...
$mysqluery=$this->db->query("select *from notification  where (student_name LIKE '%".$searchid."%' OR adhar_card LIKE '%".$searchid."%' OR roll_no LIKE '%".$searchid."%') and division='".$division."' and class_name='".$class."' and pid='".$Pid."' and notification_type='individual'  order by nid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
}else{
$div=$currentrecords[0]->divsion;
$school=$currentrecords[0]->schools_name;
$year=$currentrecords[0]->year;
$mysqluery=$this->db->query("select *from notification where division='".$division."' and class_name='".$class."' and pid='".$Pid."' and (notification_type='individual' OR  notification_type='common') order by nid desc LIMIT ".$pageLimit." , ".$setLimit)->result();
	 ...
	 <div class="row">
        ...
        <div class="panel-heading"><h3 style="color: white;font-size: 25px;text-align: center;margin-top: 6px;">Add New Notification</h3></div>
			 ...
			 <?php 
			  ...
			  <select class="form-control" name="student_name" id="student_name" onchange="student_namer();">
			  <option value="">Select</option>
			  <option value="All">All</option>
			  <?php foreach($student_get as $row){ ?>
			  <option value="<?php echo $row->Ptid;  ?>"><?php echo $row->Student_name; ?>&nbsp;(<?php echo $row->adhar_card; ?>)</option>
			  <?php } ?>
			  
			  </select>
			  <span id="student_namer" style="color:#FF0000"></span>
			  </div>
			  <div class="form-group">
			  <label>Notification</label>
		...
		<div class="row">
		<div class="col-md-6">
	<a href="<?php echo site_url(); ?>individual_notification?add-new-notification=add-new-notification" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span> Add New</a>
		    ...
		    <table id="datatable" class="table table-striped table-bordered">
                      <thead>
                        <tr>
						  ...
						  <th style="width:200px;">Aadhar / Notification Type</th>
					  ...
					  <?php $serial = ($pageLimit * $setLimit) + 1;
					   ...
					   ?>
						...
						<td><?php echo $row->adhar_card; ?> (<?php echo ucfirst($row->notification_type); ?>)</td>
...
$page_url="?";
$ci= get_instance();
$query = $ci->db->query("SELECT COUNT(*) as totalCount FROM notification  where division='".$division."' and pid='".$Pid."' and class_name='".$class."' and (notification_type='individual' OR  notification_type='common') order by nid asc")->result();
