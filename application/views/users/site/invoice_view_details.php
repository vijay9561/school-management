...
$mysqluery=$this->db->query("select i.pid,i.id,i.valid_from,i.til_date,i.payment_terms,i.registration_charges,i.software_charges,i.school_status,i.data_entry_charges,i.maintenance_charges,i.reactivation_charges,i.data_entry_charges,i.customization_charges,i.other_charges,i.pid,i.created_date,p.Principle_school_name,p.Principle_email,p.Principle_mobile_no,p.Principle_schools_city,p.address,i.invoice_no,p.Principle_name from tbl_invoice i inner join tbl_principle p on i.pid=p.Pid where (Principle_name LIKE '%".$searchid."%' OR Principle_email LIKE '%".$searchid."%' OR Principle_school_name LIKE '%".$searchid."%') and p.Pid='$pid' order by i.id desc LIMIT ".$pageLimit." , ".$setLimit)->result();
		    ...
		    <table id="datatable" class="table table-striped table-bordered">
                      ...
                      <tbody>
						...
						$num = $sn ++;
                                              ...
                                              $paybal_amount=$row->software_charges+$row->data_entry_charges+$row->maintenance_charges+$row->customization_charges+$row->reactivation_charges+$row->other_charges;
						?>
                        <tr>
						<td><?php echo $j++; ?></td>
                        ...
                        <?php if($row->school_status=='UNPAID'){ ?>
                        <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#payfees<?php echo $i; ?>"> Pay <i class="fa fa-inr"></i> <?php echo $paybal_amount; ?></a>
                        <?php }elseif($row->school_status=='PENDING'){ ?>
                        <span class="btn btn-warning">Pending</span>  
                        <?php }elseif($row->school_status=='DISAPPROVE'){
                            $mql=$this->db->query("select *from school_payment where inovice_id='$row->id'")->row();
                            ?>
                        <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#payfees<?php echo $i; ?>">Again Pay <i class="fa fa-inr"></i> <?php echo $paybal_amount; ?></a><br>
                        <a href="#" data-toggle="modal" data-target="#disaaprove<?php echo $i; ?>">Disapprove Reason</a>
                       <div class="modal" tabindex="-1" id="disaaprove<?php echo $i; ?>" role="dialog">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title">Disaaprove Reason</h5>
 ...
 <div class="modal" tabindex="-1" id="payfees<?php echo $i; ?>" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form method="post" action="<?php echo site_url(); ?>users_controller/get_payment_details">
              <input type="hidden" id="invoice_name<?php echo $i ?>" value="<?php echo base64_encode($row->id) ?>" name="invoice_name">
              <input type="hidden" id="pid<?php echo $i ?>" value="<?php echo base64_encode($row->pid) ?>" name="pid">
              <div class="form-group">
                  <label>Select Payment Mode</label>  
                  <select id="payment_mode<?php echo $i; ?>" name="payment_mode" onchange="payment_moder(<?php echo $i; ?>)" class="form-control">
                      <option value="">Select Payment Option</option> 
                      <option value="Online">Pay Via Online</option>
                      <option value="Cheque">Pay Via Cheque</option>
                  </select>
                  <span id="payment_moder<?php echo $i; ?>" class="text-danger"></span>
              </div> 
              <div class="form-group" id="divecheque<?php echo $i; ?>" style="display:none;">
                  <label>Cheque Number</label>  
                  <input type="text" class="form-control" name="cheque_no" id="cheque_no<?php echo $i; ?>">
                   <span id="cheque_nor<?php echo $i; ?>" class="text-danger"></span>
              </div> 
              <div class="form-group">
                  <input type="submit" class="btn btn-primary" onclick="return submit_data(<?php echo $i; ?>)" value="Pay Amount" >
              </div>
          </form>
      </div>
      <div class="modal-footer">
...
function submit_data(id){
   ...
   if(payment_mode==""){
    $("#payment_moder"+id).html("Please select payment option");
     return false;
   }
   if(payment_mode=="Cheque"){
   var cheque_no=$('#cheque_no'+id).val();
     if(cheque_no==""){
      $("#cheque_nor"+id).html("Please enter cheque number");
      return false;
     }
   }
