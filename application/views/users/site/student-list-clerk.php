...
}elseif((isset($_GET['class_details'])) && (isset($_GET['division_details'])) && (isset($_GET['sub_caste']))){
$class=$_GET['class_details'];
$division=$_GET['division_details'];
$sub_caste=$_GET['sub_caste'];
if((!empty($_GET['class_details'])) && (!empty($_GET['division_details'])) && (!empty($_GET['sub_caste']))){
if($class=='all'){
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and Student_class_division='$class' and divsion='$division' and sub_cast='$sub_caste' and Status='active' order by Student_name asc")->result();
}else{
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and divsion='$division' and sub_cast='$sub_caste' and Status='active' order by Student_name asc")->result();
 }
}elseif((!empty($_GET['class_details'])) && (!empty($_GET['division_details']))){
if($class=='all'){
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and divsion='$division' and Status='active' order by Student_name asc")->result();
}else{
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and Student_class_division='$class' and divsion='$division' and Status='active' order by Student_name asc")->result();
}
}elseif((!empty($_GET['class_details'])) && (!empty($_GET['sub_caste']))){
if($class=='all'){
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and sub_cast='$sub_caste' and Status='active' order by Student_name asc")->result();
}else{
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and Student_class_division='$class' and sub_cast='$sub_caste' and Status='active' order by Student_name asc")->result();
}
}else{
if($class=='all'){
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and Status='active'  order by Student_name asc")->result();
}else{
$mysqluery=$this->db->query("select *from tbl_parent where  (pid='".$currentrecords[0]->login_id."' OR pid='".$currentrecords[0]->Pid."') and Student_class_division='$class' and Status='active' order by Student_name asc")->result();
   }
  }
 }else{
...
<div class="container main">		
		...
		<div class="panel-body">
					...
					<tr><th>Admission No</th><td><?php echo $t[0]->admission_no; ?></td><td  colspan="2"></td></tr>
	   ...
	   <tr></tr>
		...
		<div class="col-md-12">
		<div class="panel panel-primary">
        <div class="panel-heading"><h3 style="color: white;font-size:18px;text-align: center;margin-top: 6px;">Student Update</h3></div>
			 <div class="panel-body">
		...
		<input type="hidden" name="Ptid" value="<?php echo $_GET['action']; ?>" />
		 ...
		 <div class="form-group col-md-3">
            ...
            <input type="text" id="admission_no" value="<?php echo $student[0]->admission_no; ?>"  onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" name="admission_no" maxlength="16" autcomplete="off" class="form-control">
		...
		<div class="form-group col-md-4">
            <label   class="control-label" for="name">Adhar Card Number <b style="color:red;">*</b></label>
            ...
            <span id="adhar_cardr" style="color:red;"></span> </div></div>
			 <div class="row">
			<div class="form-group col-md-4">
            ...
            <input type="text" id="Parent_mobile_no" name="Parent_mobile_no" value="<?php echo $student[0]->Parent_mobile_no; ?>" onchange="Parent_mobile_nor()" maxlength="50"  class="form-control">
            <span id="Parent_mobile_nor" style="color:red;"></span> </div>
			
          <div class="form-group col-md-4">
            <label   class="control-label" for="name">Roll.No <b style="color:red;">*</b></label>
            <input type="text" id="Student_roll_no" value="<?php echo $student[0]->Student_roll_no; ?>" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" name="Student_roll_no" onchange="Student_roll_nor()" maxlength="5" autcomplete="off"  class="form-control">
            <span id="Student_roll_nor" style="color:red;"></span> </div></div>
			 <div class="row">
			  <div class="form-group col-md-3">
		   ...
		   <option value="<?php echo $di->divison_name; ?>"><?php echo $di->divison_name; ?></option>	
			<?php } } ?>
			 ...
			 <span id="divsionr" style="color:red;"></span> </div>
			 	 <div class="form-group col-md-3">
            <label   class="control-label" for="name">Medium <b style="color:red;">*</b></label>
		   ...
		   <option value="<?php echo $di->name; ?>"><?php echo $di->name; ?></option>	
			...
			<div class="form-group col-md-3">
            <label   class="control-label" for="name">Class Year <b style="color:red;">*</b></label>
		   ...
		   <option value="<?php echo $di->year; ?>"><?php echo $di->year; ?></option>	
			<?php }  } ?>
			 ...
			 <span id="yearr" style="color:red;"></span> </div></div>
			  <div class="row">
          <div class="form-group col-md-4">
            ...
            <span id="mother_namer" style="color:red;"></span> </div></div>
			 <div class="row">
			<div class="form-group col-md-4">
		...
		<div class="form-group col-md-4">
            ...
            <span id="bank_nor122" style="color:red;"></span> </div>
			 ...
			 <div class="row">
             <div class="form-group col-md-4">
            <label   class="control-label" for="name">Religion (Caste)</label>
			...
			<div class="form-group col-md-4">
            <label   class="control-label" for="name">Caste Category</label>
      ...
      <input type="text" id="Subgenre"  autocomplete="off" name="Subgenre" value="<?php echo $student[0]->Subgenre; ?>"  maxlength="100" class="form-control" />
            ...
            <span id="ifc_coder" style="color:red;"></span>
			</div></div>
            <div class="row">
			<div class="form-group col-md-4">
			<label   class="control-label" for="name">Student Image</label>
...
<div class="form-group pull-right">
 <label  class="control-label" for="name">Search<b style="color:red;">*</b></label>
<input type="text" id="search_id" placeholder="" value="<?php if(isset($_GET['searchkeyowords'])) { echo $_GET['searchkeyowords']; } ?>"  style="display: initial; width:auto;" title="Type Here" class="form-control">&nbsp;&nbsp;
...
window.location='student-list-clerk?class_details='+class12+'&division_details='+division+'&sub_caste='+sub_caste;
}
		    ...
		    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  if(count($mysqluery)){
					   ...
					   ?>
						...
						<td>
					<?php if($_SESSION['USERTYPE']=='clerk'){ ?>
					   <a href="<?php echo site_url(); ?>student-list-clerk?action=<?php echo $row->Ptid; ?>" title="Edit" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
					   <?php } ?>
			 &nbsp;&nbsp;<a href="<?php echo site_url(); ?>student-list-clerk?view-details=<?php echo $row->Ptid; ?>" title="View" class="btn btn-success"><i class="fa fa-eye"></i></a>
...
$page_url="?";
$ci= get_instance();
$query = $ci->db->query("SELECT COUNT(*) as totalCount FROM tbl_parent  where (pid='".$login."' OR pid='".$login."') and Status='active'")->result();
