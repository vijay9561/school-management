...
<div class="container main">			
	   ...
	   <tr></tr>
		...
		<thead><tr><th>Sr No</th><th>Fees Type</th><th>Paid Fees</th><th>Fees Paid Date</th></tr></thead>
		<tbody>
		<tr>
		<td>1</td>
		<td><select type="text" required name="fees_type[]"  placeholder="Fees Type *"  class="form-control" required>
		<option value="">Select Fees Type</option>
		<option value="School Fees">School Fees</option>
       <option value="Examination Fees">Examination Fees</option>
		<option value="Tour Fees">Tour Fees</option>
	   ...
	   <option value="Other Fees">Other Fees</option>
		...
		<?php }elseif(isset($_GET['update_amount'])){
				 ...
				 $sumvalues=$this->db->query("select sum(paid_fees) as paidfees from tbl_fees_type where fees_id='".$fees->id."' and fees_type='School Fees'")->result();
		 ?>
		<div class="row">
		<div class="col-md-12">
		<div class="panel panel-primary">
		...
		<div class="row">
		<table class="table table-bordered">
		<thead><tr><th>Sr No</th><th>Fees Type</th><th>Paid Fees</th><th>Paid Date</th></tr></thead>
		<tbody>
		<tr>
		...
		<td>
		<select type="text" required name="fees_type[]"  placeholder="Fees Type *"  class="form-control" required>
	...
	<input type="text" required name="created_date[]"  value="<?php if($row->created_date!='0000-00-00'){ echo ($row->created_date); } ?>" autocomplete="off" placeholder="Paid Date" class="form-control datepicker" required />
		...
		<tr>
		<td><?php echo $i; ?></td>
		<td><select type="text" required name="fees_type[]"  placeholder="Fees Type *"  class="form-control">
		<option value="<?php echo $row->fees_type; ?>"><?php echo $row->fees_type; ?></option>
		<option value="School Fees">School Fees</option>
       <option value="Examination Fees">Examination Fees</option>
		<option value="Tour Fees">Tour Fees</option>
	...
	<input type="text"  name="created_date[]" value="<?php if($row->created_date!='0000-00-00'){ echo ($row->created_date); } ?>" autocomplete="off" placeholder="Paid Date" class="form-control datepicker"  />
	   ...
	   <option value="Other Fees">Other Fees</option>
		...
		$fees=$this->db->query("select sum(paid_fees) as totalfees from tbl_fees_type where fees_id='$id' and fees_type='School Fees'")->result();
		?>
		<div class="row">
		<div class="col-md-12">
		<div class="panel panel-primary">
        <div class="panel-heading"><h3 style="color: white;font-size:18px;text-align: center;margin-top: 6px;">View Fees Details</h3></div>
			...
			<tr><th>Sr.No</th><th>Fees Type</th><th>Paid Fees</th><th>Paid Date</th><th></th></tr>
		    ...
		    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  if(count($mysqluery)){
					   ...
					   ?>
						...
						<?php
						 $fees_id=$tbl_fees->id;
			 $paid_fees1=$this->db->query("SELECT SUM(paid_fees) AS total_paid_amount FROM tbl_fees_type where fees_id='".$tbl_fees->id."' and fees_type='School Fees'")->result();
